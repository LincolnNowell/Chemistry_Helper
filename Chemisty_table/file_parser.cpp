...
int main()
{
    ...
    for(size_t character = 0; character < line.size(); character++){
        ...
        else if(character > 0 and line.at(character - 1) != exponent_symbol and line.at(character - 1) != ' '){
            if(line.at(character) > 47 and line.at(character) < 58){
                store = line.at(character);
        ...
        else if(line.at(character) > 47 and line.at(character) < 58 and chang_constant == true){
            store = line.at(character);
            numb_elements = std::stoi(store);
            chang_constant = false;
        }
        ...
        if(line.at(character) > 64 and line.at(character) < 91){
            ...
            if(line.at(character + 1) > 47 and line.at(character + 1) < 58){
                store = line.at(character + 1);
                subscript = std::stoi(store);
                create_element = true;
            }
            if(line.at(character + 1) < 97 and line.at(character + 1) > 123 and line.at(character + 1) != exponent){
                create_element = true;
            }
        }

        //check to see if to see if character is a lowercase
        if(line.at(character) > 97 and line.at(character) < 123){
            name += line.at(character);
            if(line.at(character + 1) == exponent_symbol){
