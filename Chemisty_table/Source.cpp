...
int getConstant(std::map<int, int>);
...
void balance(equation equation_to_balance) {
	...
	char var = 'A';
	for (auto& Cmpd : total) { Cmpd.mVariable = var++; }
	...
	for (size_t element = 0; element < equation_to_balance.left.size(); element++)
	{
		for (size_t compound = 0; compound < equation_to_balance.left[element].size; compound++)
		{
				...
				for (size_t inner_element = 0; inner_element < total[inner_compound].size; inner_element++)
				{
						...
						total[inner_compound].compound_elements[inner_element].name) 
					{
						if (total[inner_compound].Constant != 0) {
							equation += total[inner_compound].Constant +
								"(" + std::to_string(total[inner_compound].compound_elements[inner_element].numb_of_elements) + ")";
						}
						else {
			...
			std::cout << equation << " ";
			//call function to solve for constants
			equation = "";
		}
	}
...
int getConstant(std::map<int, int> equation) {
